...
task.spawn(function()
    ...
    local function ApplyPremiumStyle(instance)
        if instance:IsA("TextLabel") or instance:IsA("TextButton") or instance:IsA("TextBox") then
            instance.Font = Enum.Font.GothamMedium
            if instance.TextSize and instance.TextSize < 15 then
                instance.TextSize = 15
            end
        end
        
        if instance:IsA("Frame") and instance.BackgroundColor3 \~= Color3.new(1,1,1) then
            if instance.Size.Y.Offset > 10 and instance.Size.X.Offset > 10 then
    ...
    for _, gui in ipairs(coreGui:GetChildren()) do
        if gui:IsA("ScreenGui") then
            ...
            if isTarget then
                ...
                gui.DescendantAdded:Connect(ApplyPremiumStyle)
            end
        end
    end
end)
...
local MainBOX = GeneralTab:AddLeftTabbox("Main") do
    ...
    Main:AddDropdown("Method", {AllowNull = true, Text = "Silent Aim Method", Default = SilentAimSettings.SilentAimMethod, Values = {
        "Raycast","FindPartOnRay","FindPartOnRayWithWhitelist","FindPartOnRayWithIgnoreList","Mouse.Hit/Target"
    }}):OnChanged(function() 
        SilentAimSettings.SilentAimMethod = Options.Method.Value 
    end)
    Main:AddSlider('HitChance', {Text = 'Hit chance', Default = 100, Min = 0, Max = 100, Rounding = 1, Compact = false}):OnChanged(function()
...
local FieldOfViewBOX = GeneralTab:AddLeftTabbox("Field Of View") do
    ...
    Main:AddToggle("Visible", {Text = "Show FOV Circle"}):AddColorPicker("Color", {Default = Color3.fromRGB(0, 255, 255)}):OnChanged(function()
    ...
    Main:AddToggle("MousePosition", {Text = "Show Silent Aim Target"}):AddColorPicker("MouseVisualizeColor", {Default = Color3.fromRGB(0, 255, 255)}):OnChanged(function()
